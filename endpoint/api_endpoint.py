    ...
    try:
        ...
        while True:
            ...
            if time_diff > timedelta(hours=5):
                print(f"最後にアクセスがあってから、5時間経過しました。無限ループの可能性を考え処理を自動停止します。 起動時:{start_activate_time}\n最後のアクセス:{last_access_time}")
                stop_this_instance(instance_id)
                await asyncio.sleep(15)
                print(f"直ぐにエンジニアの確認が必要です。モニターによる自動停止に失敗しました。 起動時:{start_activate_time}\n最後のアクセス:{last_access_time}")
                #ここにslackに通知する機能を追加する。

            elif not is_processing and active_time_diff > timedelta(minutes=15):
                print(f"最後の処理終了から15分以上経過しました。インスタンスを停止します。 起動時:{start_activate_time}\n最後の処理終了:{last_active_time}")
                stop_this_instance(instance_id)
                await asyncio.sleep(15)
                print(f"直ぐにエンジニアの確認が必要です。モニターによる自動停止に失敗しました。 起動時:{start_activate_time}\n最後のアクセス:{last_access_time}")
        ...
        try:
            print(e)
            print(f"エンジニアの確認が必要です。モニタリングに予期せぬエラーが発生しました。インスタンスを停止します。エラー:{e}")
            stop_this_instance(instance_id)
        except:
            print(f"直ぐにエンジニアに確認して下さい。stop_this_instance関数に問題があります。インスタンスの停止に失敗しました。")
...
async def start_monitoring():
    ...
    print("モニタリングが開始されました。リクエストを受け付けます。")
...
async def endpoint_of_all(request: Request, call_next):
    ...
    try:
        ...
        # ロック取得成功
        try:
            print(f"リクエストを受け付けました。")
            is_processing = True  # 処理中フラグを有効化
            last_access_time = datetime.now(JST)  # 現在の日本時間を記録
            response = await call_next(request)
            return response
        except Exception as e:
            print(f"ミドルウェアでエラーが発生しました。: {e}")
        ...
        # その他の例外をキャッチ
        print(f"ミドルウェアで未処理のエラーが発生しました: {e}")
...
async def process_feach_japanese(request_data: RequestData):
    try:
        ...
        if request_data.stop_control:
            print(f"インスタンスを停止します。インスタンスID:{this_instance_id}")
