...
class Outputer:
    ...
    async def batch_execute_for_display(self,output_mode=""):
        try:
            ...
            #ソーシングツールで使用する出力方法
                ...
                header,results_list = self.__adjust_indicators_for_simple(self.results_list)
            ...
            #ソーシングツールで使用する出力方法
                ...
                header,results_list = self.__adjust_indicators_sourcing(self.results_list)
    ...
    def __adjust_indicators_sourcing(self,dict_list):
        ...
        need_keys = [ # 必要なキー
            ...
            "論文1:ID","論文1:タイトル","論文1:出版年月","論文1:被引用数","predict_model"
        ]
    ...
    def __adjust_indicators_for_simple(self, dict_list):
        new_list = []
        header = ["研究者ID", "名前", "最新の所属", "予測モデル", "H-index", "キャリア年数", "出版数", "総被引用数", "top3_topics"]
        ...
        # dict_list 内の各辞書について処理
        for original_dict in dict_list:
            # 必要なキーのみ抽出
            new_dict = {key: original_dict[key] for key in need_keys if key in original_dict}
            
            # latest_affiliation がリストの場合、先頭要素のみを抽出
            if "latest_affiliation" in new_dict and isinstance(new_dict["latest_affiliation"], list):
                new_dict["latest_affiliation"] = new_dict["latest_affiliation"][0] if new_dict["latest_affiliation"] else ""
            
            # topics_detail がリストの場合、先頭3件をカンマ区切りの文字列に変換
            if "topics_detail" in new_dict and isinstance(new_dict["topics_detail"], list):
                new_dict["topics_detail"] = ",".join([str(topic.get("Display Name", "")) for topic in new_dict["topics_detail"][:3]])
    ...
    def prepend_five_evaluation(self,header,dict_list):
        try:
            header = ["研究の質","h-index世界ランク","若さ","革新性","予測モデル"]+header
            new_list = []
            for item in dict_list:
                
                #"検索リンク"は、不要そうなのでコメントアウト中。必要だったら追加。
